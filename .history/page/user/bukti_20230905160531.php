...
if (in_array($extension, $cek) === TRUE){
  if ($size_bukti < 5044070){
    ...
    $query = $conn->prepare("SELECT * FROM tbl_pesanan WHERE id_pesanan =:id ");
    ...
    try {
			...
			$insertdata = array(':nama_bukti' => $name_bukti, ':type_bukti' => $type_bukti, ':size_bukti' => $size_bukti,':id_pesanan' => $id_pesanan);
