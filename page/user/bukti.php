...
if (in_array($extension, $cek) === TRUE){
  if ($size_bukti < 5044070){
    ...
    try {
			...
			$pdo = $conn->prepare('INSERT INTO tbl_ongkir (id_user, nama_bukti, type_bukti, size_bukti)
			values (:id_user, :nama_bukti, :type_bukti, :size_bukti)');

			$insertdata = array(':nama_bukti' => $name_bukti, ':type_bukti' => $type_bukti, ':size_bukti' => $size_bukti,':id_user' => $id);
